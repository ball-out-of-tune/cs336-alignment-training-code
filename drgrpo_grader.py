...
# 假设这些是你项目中已经存在的工具函数
# from drgrpo_grader import grade, extract_boxed_answer 

def r1_zero_reward_fn_loose(response: str, ground_truth: Union[str, List[str], float, int], fast: bool = True) -> Dict[str, float]:
    """
    针对 extract_boxed_answer 和 grade 函数定制的宽松奖励函数。
    """
    
    # --- 1. 格式评分 (Format Reward) ---
    # 只要出现了 <answer>...</answer> 标签对，就认为格式及格
    # 不强制要求 <think> 标签的位置或空格
    format_ok = "<answer>" in response and "</answer>" in response
    ...
    # 策略 A: 优先尝试从 <answer> 标签内部提取
    if format_ok:
        try:
            # 提取标签中间的内容
            content = response.split("<answer>")[-1].split("</answer>")[0]
            
            # 尝试利用 extract_boxed_answer 提取 \boxed{}
            # 注意：如果 content 里没有 \boxed，last_boxed_only_string 通常返回 None
            extracted = extract_boxed_answer(content)
            
            if extracted is not None:
                model_answer = extracted
            else:
                # 如果标签内没有 boxed，尝试直接使用内容作为答案（应对未打框的情况）
                # strip() 去除首尾空白，防止空字符串进入 grade
    ...
    # 策略 B: 兜底策略 (Fallback)
    # 如果策略 A 没拿到答案（比如格式坏了，或者标签里全是废话），
    # 则在 *全文* 中搜索最后一个 \boxed{}。
    # 既然 extract_boxed_answer 内部调用了 last_boxed_only_string，它会自动找最后一个。
    if model_answer is None:
        extracted = extract_boxed_answer(response)
        if extracted is not None:
            model_answer = extracted

    # --- 3. 正确性评分 (Correctness Check) ---
    answer_reward = 0.0
    total_reward = 0.0
    
    # 只有当提取到了有效字符串，才调用 grade，防止报错
    if model_answer is not None:
        ...
        try:
            if isinstance(ground_truth, str):
                ...
                # 列表情况：只要匹配中任何一个 GT 就算对
                for gt in ground_truth:
                    if grade(model_answer, gt, fast):
                        is_correct = True
                        break
        except Exception as e:
            # 极少数情况下 grade 内部可能会崩，这里做个防护，算错处理
            print(f"Warning: grade function failed: {e}")
            is_correct = False
        
        if is_correct:
            answer_reward = 1.0
            total_reward = 1.0 # 宽松模式：只要答案对，总分就给满
